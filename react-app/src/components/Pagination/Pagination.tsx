...
export const Pagination: React.FC<PaginationProps> = ({ onPageChange, currentPage, totalPages }) => {
    ...
    return (
        <P.Wrapper>
            <P.Button isCurrentPage={currentPage === 1} disabled={currentPage === 1} onClick={(e) => onPageChange(1)}>1</P.Button>
            ...
            {(totalPages > 2) && (currentPage > 1) && (currentPage < totalPages) && (<P.Button disabled isCurrentPage>{currentPage}</P.Button>)}
            ...
            {(totalPages > 1) && (<P.Button isCurrentPage={currentPage === totalPages} disabled={currentPage === totalPages} onClick={(e) => onPageChange(totalPages)}>{totalPages}</P.Button>)}
        </P.Wrapper>
    );
};
